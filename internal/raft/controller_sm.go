...
// for managing cluster metadata - simplified version for demo
...
type PartitionAssignment struct {
	...
	PreferredLeader string   `json:"preferred_leader"` // Controller specified preferred leader
}
...
func (csm *ControllerStateMachine) Update(data []byte) (statemachine.Result, error) {
	...
	// Increment version and update time
...
func (csm *ControllerStateMachine) executeCommand(cmd *ControllerCommand) (interface{}, error) {
	...
	case "update_broker_load":
		return csm.updateBrokerLoad(cmd.Data)
	case "mark_broker_failed":
...
// Basic implementations for other commands - simplified for demo
...
func (csm *ControllerStateMachine) createTopic(data map[string]interface{}) (interface{}, error) {
	...
	// Simple topic creation - in real implementation this would be more complex
	topic := &TopicMetadata{
		Name:              topicName,
...
func (csm *ControllerStateMachine) Lookup(query interface{}) (interface{}, error) {
	...
	case "get_partition_assignments":
...
func (csm *ControllerStateMachine) GetHash() uint64 {
	...
	// Return a simple hash based on version for now
