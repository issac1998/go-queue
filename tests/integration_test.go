...
// TestTransactionProducer 测试事务生产者功能
func TestTransactionProducer(t *testing.T) {
	...
	// 创建测试Topic
	topicName := fmt.Sprintf("tx-test-topic-%d", time.Now().Unix())
	_, err = admin.CreateTopic(client.CreateTopicRequest{
		Name:       topicName,
		Partitions: 1,
		Replicas:   1,
	...
	// 创建事务生产者，使用简单的mock listener
	mockListener := &MockTransactionListener{}
	txProducer := client.NewTransactionProducer(c, mockListener)

	// 开始事务
	tx, err := txProducer.BeginTransaction()
	if err != nil {
		t.Fatalf("Failed to begin transaction: %v", err)
	}

	// 在事务中发送消息
	txMsg := &client.TransactionMessage{
		Topic:     topicName,
		Partition: 0,
		Value:     []byte("Transaction message 1"),
		Timeout:   5 * time.Second,
	}

	_, err = tx.SendHalfMessageAndDoLocal(txMsg)
	if err != nil {
		t.Fatalf("Failed to send transaction message: %v", err)
	}
	...
	t.Logf("Transaction committed successfully: %+v", commitResult)

	// 验证消息已提交
	consumer := client.NewConsumer(c)
	fetchResult, err := consumer.FetchFrom(topicName, 0, 0)
	if err != nil {
		t.Fatalf("Failed to fetch transaction message: %v", err)
	}

	if len(fetchResult.Messages) == 0 {
		t.Fatal("Transaction message not found after commit")
	}

	if string(fetchResult.Messages[0].Value) != "Transaction message 1" {
		t.Fatalf("Transaction message content mismatch")
	}

	t.Logf("Transaction producer test completed successfully")
}
